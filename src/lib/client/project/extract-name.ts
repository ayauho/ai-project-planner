...
export function extractProjectName(input: string): NameExtractionResult {
  logger.debug('Extracting project name from input', { inputLength: input.length }, 'project-management name-extraction');
  ...
  if (input.includes(NAME_EXTRACTION_RULES.DASH_SEPARATOR)) {
    ...
    if (name && description) {
      ...
      if (trimmedName.length <= NAME_EXTRACTION_RULES.MAX_NAME_LENGTH) {
        logger.debug('Extracted name using slash separator', { name: trimmedName }, 'project-management name-extraction slash-format');
  ...
  if (sentenceMatch) {
    ...
    if (trimmedName && description && trimmedName.length <= NAME_EXTRACTION_RULES.MAX_NAME_LENGTH) {
      logger.debug('Extracted name using sentence format', { name: trimmedName }, 'project-management name-extraction sentence-format');
  ...
  if (input.includes(NAME_EXTRACTION_RULES.LINE_SEPARATOR)) {
    ...
    if (trimmedName && description && trimmedName.length <= NAME_EXTRACTION_RULES.MAX_NAME_LENGTH) {
      logger.debug('Extracted name using new line separator', { name: trimmedName }, 'project-management name-extraction newline-format');
      return { name: trimmedName, description };
    }
  }

  // If no separator found, use entire input as description
  logger.debug('No separator found, using entire input as description', {}, 'project-management name-extraction fallback');
  return { 
    name: 'New Project', 
